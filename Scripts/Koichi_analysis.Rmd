...
```{r}
RNA_DM_enhancer_Down <- RNAseq_Baselin_rownames[which(rownames(RNAseq_Baselin_rownames) %in% Genes_DM_enhancer_),]
colnames(RNA_DM_enhancer_DE) <- substring(colnames(RNA_DM_enhancer_DE), 2,11)
...
Make_heatmap(RNA_DE, Phenotype = tmp, "pearson", "Non_responder & Responder based on Genes DE", ann_color_RNA)
```

```{r}
pheatmap(as.matrix(RNA_DE),
...
msviper_results$regulons %>% write.csv("../Results/regulons_msviper.csv")
...
Gene_annotations <- merge(Gene_annotations, Tf_activities_phenotype, by.x = "Gene_name", by.y = 0, all =)
...
write.csv(Gene_annotations, "../Results/Genes_values.csv")
```
