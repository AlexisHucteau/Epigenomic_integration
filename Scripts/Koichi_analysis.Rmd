...
```{r}
Sample_Baseline_NR <- dplyr::filter(Phenotype_Wang_Feng_RNAseq, Phenotype == "Baseline" & RUNX1 == "RUNX1_wt" & Non_Responder == "TRUE") %>% .$Sample
...
tf_activities_Relapse %>% as.data.frame() %>% .[unique(c(Sample_relapse_no_RUNX1, Sample_Baseline_R)),] %>% dplyr::select(c("RUNX1", "CEBPA")) %>%
  as.matrix() %>% 
  pheatmap(main="TF activities Relapse",
    show_rownames = FALSE,
    clustering_distance_rows="euclidean",
