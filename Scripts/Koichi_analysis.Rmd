...
gene_rownames <- rownames(RNAseq_Wang_Feng) %>% str_split(., "[|]") %>% lapply(., function(x){
  x[1]
}) %>%
  unlist(.) %>% 
  unique(.)
...
# RUNX1 analysis
```{r}
RUNX1_profile <- read.table("~/GitHub/Epigenomic_integration/DATA/RUNX1_profile.txt", quote="\"", comment.char="")
RUNX1_profile$Sample_name <- paste0(RUNX1_profile$V1, "_BL")
RUNX1_profile$Sample_name2 <- paste0(RUNX1_profile$V1, "_REL")

Phenotype_Wang_Feng_RNAseq$RUNX1 <- ifelse((Phenotype_Wang_Feng_RNAseq$Sample %in% RUNX1_profile$Sample_name | Phenotype_Wang_Feng_RNAseq$Sample %in% RUNX1_profile$Sample_name2), "RUNX1_mut", "RUNX1_wt")
Phenotype_Wang_Feng_RNAseq_merged <- merge(Phenotype_Wang_Feng_RNAseq, tmp, by.x = "Sample", by.y = 0, all = T)


annotation_RUNX1 <- list(
  Response = c(Responder = "blue", Non_responder = "red"),
  RUNX1_mutation = c(RUNX1_mut = "red", RUNX1_wt = "blue"))

annotation_for_heatmap_RUNX1 <- data.frame(RUNX1_mutation = Phenotype_Wang_Feng_RNAseq$RUNX1, Response = Phenotype_Wang_Feng_RNAseq$Phenotype.y)
rownames(annotation_for_heatmap_RUNX1) <- Phenotype_Wang_Feng_RNAseq$Sample

annotation_for_heatmap_RUNX1 <- annotation_for_heatmap_RUNX1[-which(is.na(annotation_for_heatmap_RUNX1$Response)),]

RUNX1_activity <- dplyr::select(tf_activities, c("RUNX1", "CEBPA"))

png("../Results/RUNX1_activity_mutation.png", width = 360*(16/9), height = 360)
pheatmap(as.matrix(RUNX1_activity),
  main="TF activities",
  show_rownames = FALSE,
  clustering_distance_rows="euclidean",
  scale = "column", 
  annotation_row = annotation_for_heatmap_RUNX1,
  annotation_colors = annotation_RUNX1
  )
dev.off()
pheatmap(as.matrix(RUNX1_activity),
  main="TF activities",
...
RUNX1_exp <- voom(RNAseq_Baselin_rownames)$E %>% .[c("RUNX1", "CEBPA"),] %>% t()
RUNX1_exp_activity <- merge(RUNX1_exp, RUNX1_activity, by.x = 0, by.y = 0) 
rownames(RUNX1_exp_activity) <- RUNX1_exp_activity$Row.names
RUNX1_exp_activity <- RUNX1_exp_activity[,-1]
colnames(RUNX1_exp_activity) <- c("RUNX1 exp", "CEBPa exp", "RUNX1 activity", "CEBPA activity")

png("../Results/RUNX1_activity_expression_mutation.png", width = 360*(16/9), height = 360)
RUNX1_exp_activity %>% as.matrix() %>% 
  pheatmap(main="RUNX1 CEBPA expression & activity",
    show_rownames = FALSE,
    clustering_distance_rows="euclidean",
    scale = "column", 
    annotation_row = annotation_for_heatmap_RUNX1,
...
```{r}
fisher_table_RUNX1 <- data.frame(RUNX1_mutation = c(5, 2), Response = c(11, 10))
fisher.test(fisher_table_RUNX1)
```

```{r}
RUNX1_high_CEBPA_down <- RUNX1_activity %>% dplyr::filter(RUNX1 > 0 & CEBPA < 0) %>% rownames()
RUNX1_down_CEBPA_up <- RUNX1_activity %>% dplyr::filter(RUNX1 < 0 & CEBPA > 0) %>% rownames()
```

```{r}
RUNX1_CEBPA_focus <- voom(RNAseq_Baselin_rownames)$E %>%
  .[,which(colnames(.) %in% c(RUNX1_high_CEBPA_down, RUNX1_down_CEBPA_up))]

Phenotype_RUNX1_CEBPA <- ifelse(colnames(RUNX1_CEBPA_focus) %in% RUNX1_high_CEBPA_down, "RUNX1_up_CEBPA_down", "RUNX1_down_CEBPA_up")

RUNX1_CEBPA_Analysis <- Differential_analysis(Phenotype_RUNX1_CEBPA, RUNX1_CEBPA_focus)
```

```{r}
volcanoplot_gene_expression(RUNX1_CEBPA_Analysis[["RUNX1_down_CEBPA_up-RUNX1_up_CEBPA_down"]], "Differential gene expression based on RUNX1/CEBPA activity")
```

# Pathway VDR enrichi quand CEBPa actif vs CEBPa inactif chez KOICHI

```{r}
Baseline_samples_CEBPa_actif <- RUNX1_activity %>% dplyr::filter(CEBPA > 0) %>% rownames()
Baseline_samples_CEBPa_inactif <- RUNX1_activity %>% dplyr::filter(CEBPA < 0) %>% rownames()
```

```{r}
Phenotype_CEBPA <- ifelse(colnames(RNAseq_Baselin_rownames) %in% Baseline_samples_CEBPa_actif, "ACTIF", ifelse(colnames(RNAseq_Baselin_rownames) %in% Baseline_samples_CEBPa_inactif, "INACTIF", "NA"))

CEBPa_activity_impact_on_GE <- Differential_analysis(Phenotype_CEBPA, voom(RNAseq_Baselin_rownames)$E)
```
```{r}
KEGG_analysis_CEBPA_activity <- KEGG_pathway_analysis(CEBPa_activity_impact_on_GE[["ACTIF-INACTIF"]], organism = org.Hs.eg.db)
```

```{r}
Gene_set_CEBPA_activity <- Gene_set_analysis(CEBPa_activity_impact_on_GE[["ACTIF-INACTIF"]], organism = org.Hs.eg.db)
```

# NR vs R without RUNX1m

```{r}
RNAseq_no_RUNX1m <- RNAseq_Baselin_rownames %>% voom(.) %>% .$E %>% .[,-which(colnames(.) %in% RUNX1_profile$Sample_name)]

NR_Samples <- Phenotype_Wang_Feng_RNAseq %>% dplyr::filter(Phenotype == "Baseline" & Non_Responder == TRUE) %>% .$Sample
R_Samples <-  Phenotype_Wang_Feng_RNAseq %>% dplyr::filter(Phenotype == "Baseline" & Responder == TRUE) %>% .$Sample

Phenotype_NR_R_no_RUNX1 <- ifelse(colnames(RNAseq_no_RUNX1m) %in% NR_Samples, "Non_Responder", ifelse(colnames(RNAseq_no_RUNX1m) %in% R_Samples, "Responder", "NA"))

RNAseq_no_RUNX1m_analysis <- Differential_analysis(Phenotype_NR_R_no_RUNX1, RNAseq_no_RUNX1m)

write.csv(RNAseq_no_RUNX1m_analysis[["Non_Responder-Responder"]], "../Results/Non_Responder_Responder_no_RUNX1m_analysis.csv")
```

```{r}
KEGG_analysis_NR_R_no_RUNX1 <- KEGG_pathway_analysis(RNAseq_no_RUNX1m_analysis[["Non_Responder-Responder"]], organism = org.Hs.eg.db)
```

```{r}
Gene_set_NR_R_no_RUNX1 <- Gene_set_analysis(RNAseq_no_RUNX1m_analysis[["Non_Responder-Responder"]])
```

```{r}
gseaplot(Gene_set_NR_R_no_RUNX1, by = "all", title = Gene_set_NR_R_no_RUNX1$Description[723], geneSetID = 723)
```

```{r}
A <- rownames(tf_activities) %in% Sample_Baseline_NR_RUNX1wt
B <- rownames(tf_activities) %in% Sample_Baseline_R_RUNX1wt 

tf_Ttest_activities_NR_vs_R_no_RUNX1 <- tf_activities %>%
  sapply(., function(x){
    test_tmp <- t.test(x[A], x[B])
    c("p.value_activities" = test_tmp$p.value, 
               "Non_reponder" = test_tmp$estimate[1], 
               "Responder" = test_tmp$estimate[2])
  }) %>%
  t()

write.csv(tf_Ttest_activities_NR_vs_R_no_RUNX1, "../Results/tf_Ttest_activities_NR_vs_R_no_RUNX1.csv")
```


```{r}
tf_activities_No_RUNX1m <- run_viper(RNAseq, regulons, 
                           options =  list(method = "scale", minsize = 4, eset.filter = FALSE, cores = 1, verbose = FALSE))
tf_activities_No_RUNX1m=t(tf_activities_No_RUNX1m) %>% as.data.frame()
```


```{r}
png("../Results/RUNX1_CEBPA_activity_no_RUNX1m.png", width = 360*(16/9), height = 360)
tf_activities_No_RUNX1m %>% dplyr::select(c("RUNX1", "CEBPA")) %>%
  as.matrix() %>% 
  pheatmap(main="TF activities no RUNX1m",
    show_rownames = FALSE,
    clustering_distance_rows="euclidean",
    scale = "column", 
    annotation_row = annotation_for_heatmap_RUNX1,
...
png("../Results/TF_activity_no_RUNX1m.png", width = 360*(16/9), height = 360)
tf_activities_No_RUNX1m %>% 
  as.matrix() %>% 
  pheatmap(main="TF activities no RUNX1m",
    show_rownames = FALSE,
    clustering_distance_rows="euclidean",
...
diff_TF <- tf_Ttest_activities_No_RUNX1m %>% as.data.frame() %>% dplyr::filter(p.value_activities < 0.1) %>% rownames(.)

png("../Results/TF_activity_diff_no_RUNX1m.png", width = 360*(16/9), height = 360)
tf_activities_No_RUNX1m %>% dplyr::select(diff_TF) %>%
  as.matrix() %>% 
  pheatmap(main="TF activities no RUNX1m",
    show_rownames = FALSE,
    clustering_distance_rows="euclidean",
...
```{r}
Sample_Baseline_NR_RUNX1wt <- dplyr::filter(Phenotype_Wang_Feng_RNAseq, Phenotype == "Baseline" & RUNX1 == "RUNX1_wt" & Non_Responder == "TRUE") %>% .$Sample
Sample_Baseline_R_RUNX1wt <- dplyr::filter(Phenotype_Wang_Feng_RNAseq, Phenotype == "Baseline" & RUNX1 == "RUNX1_wt" & Responder == "TRUE") %>% .$Sample

A <- rownames(tf_activities_No_RUNX1m) %in% Sample_Baseline_NR

B <- rownames(tf_activities_No_RUNX1m) %in% Sample_Baseline_R_RUNX1wt

tf_Ttest_activities_No_RUNX1m <- tf_activities_No_RUNX1m %>%
  sapply(., function(x){
    test_tmp <- t.test(x[A], x[B])
    c("p.value_activities" = test_tmp$p.value, 
               "Non_reponder_mean" = test_tmp$estimate[1], 
...
```{r}
Genes_values_No_RUNX1m <- merge(tf_Ttest_activities_No_RUNX1m, RNAseq_no_RUNX1m_analysis[["Non_Responder-Responder"]], by.x = 0, by.y = "ID", all = T)

Genes_values_No_RUNX1m <- dplyr::mutate_at(Genes_values_No_RUNX1m, c(2), funs(replace(., is.na(.), 1)))
Genes_values_No_RUNX1m <- dplyr::mutate_at(Genes_values_No_RUNX1m, c(3, 4), funs(replace(., is.na(.), 0)))

Genes_values_No_RUNX1m_Responder_Relapse <- merge(Genes_values_No_RUNX1m, tf_Ttest_activities_Responser, by.x = "Row.names", by.y = 0, all = T)

Genes_values_No_RUNX1m_Responder_Relapse <- dplyr::mutate_at(Genes_values_No_RUNX1m_Responder_Relapse, c(11), funs(replace(., is.na(.), 1)))
Genes_values_No_RUNX1m_Responder_Relapse <- dplyr::mutate_at(Genes_values_No_RUNX1m_Responder_Relapse, c(12, 13), funs(replace(., is.na(.), 0)))

write.csv(Genes_values_No_RUNX1m_Responder_Relapse, "../Results/Genes_values_No_RUNX1m.csv", row.names = F)
...
```{r}
Sample_NR_no_RUNX1 <- dplyr::filter(Phenotype_Wang_Feng_RNAseq, Phenotype == "Baseline" & RUNX1 == "RUNX1_wt" & Non_Responder == "TRUE") %>% .$Sample
Sample_relapse_no_RUNX1 <- dplyr::filter(Phenotype_Wang_Feng_RNAseq, Phenotype == "Relapse" & RUNX1 == "RUNX1_wt") %>% .$Sample

RNAseq_Relapse <- RNAseq_Wang_Feng %>% voom() %>% .$E %>%as.data.frame()

RNAseq_Relapse$Gene <- rownames(RNAseq_Relapse) %>% str_split(., "[|]") %>% lapply(., function(x){
  x[1]
}) %>%
  unlist(.)

RNAseq_Relapse <- RNAseq_Relapse %>%
  split(., .$Gene) %>%
  lapply(., function(x){
    l <- length(x[1,]) - 1
    cnames <- colnames(x)[c(1:l)]
...
gene_rownames <- rownames(RNAseq_Wang_Feng) %>% str_split(., "[|]") %>% lapply(., function(x){
  ...
  unlist(.) %>% 
  unique(.)
rownames(RNAseq_Relapse) <- gene_rownames

colnames(RNAseq_Relapse) <- substr(colnames(RNAseq_Relapse), 2, 12)

Phenotype_NR_R_Relapse <- ifelse(colnames(RNAseq_Relapse) %in% Sample_NR_no_RUNX1, "Non_Responder", ifelse(colnames(RNAseq_Relapse) %in% Sample_relapse_R_no_RUNX1, "Responder", "NA"))


RNAseq_Relapse_analysis <- Differential_analysis(Phenotype_NR_R_Relapse, RNAseq_Relapse)
```

```{r}
tf_activities_Relapse <- run_viper(RNAseq_Relapse, regulons, 
                           options =  list(method = "scale", minsize = 4, eset.filter = FALSE, cores = 1, verbose = FALSE))
tf_activities_Relapse=t(tf_activities_Relapse) %>% as.data.frame()
```

```{r}
A <- rownames(tf_activities_Relapse) %in% Sample_NR_no_RUNX1

B <- rownames(tf_activities_Relapse) %in% Sample_Baseline_R_RUNX1wt

tf_Ttest_activities_Relapse <- tf_activities_Relapse %>%
  sapply(., function(x){
    test_tmp <- t.test(x[A], x[B])
    c("p.value_activities" = test_tmp$p.value, 
               "Non_reponder" = test_tmp$estimate[1], 
               "Relapse" = test_tmp$estimate[2])
  }) %>%
  t()
```
...
```{r}
A <- rownames(tf_activities_Relapse) %in% Sample_relapse_no_RUNX1

B <- rownames(tf_activities_Relapse) %in% Sample_Baseline_R_RUNX1wt

tf_Ttest_activities_Responser <- tf_activities_No_RUNX1m %>%
  sapply(., function(x){
    test_tmp <- t.test(x[A], x[B])
    c("p.value_activities" = test_tmp$p.value, 
               "Responser at relapse" = test_tmp$estimate[1], 
               "Responder at Baseline" = test_tmp$estimate[2])
  }) %>%
  t()
```

```{r}
annotation_color_Relapse <- list(
  Response = c(Responder = "blue", Non_Responder = "red", Relapse_or_No_data = "grey"),
  RUNX1_mutation = c(RUNX1_mut = "red", RUNX1_wt = "blue"),
  Relapse = c(Baseline = "blue", Relapse = "red"))

Response <- ifelse(Phenotype_Wang_Feng_RNAseq$Responder, "Responder", ifelse(Phenotype_Wang_Feng_RNAseq$Non_Responder, "Non_Responder", "Relapse_or_No_data"))

annotation_for_heatmap_Relapse <- data.frame(Sample = Phenotype_Wang_Feng_RNAseq$Sample,
                                             Response = Response,
                                             Relapse = Phenotype_Wang_Feng_RNAseq$Phenotype,
                                             RUNX1_mutation = Phenotype_Wang_Feng_RNAseq$RUNX1)
rownames(annotation_for_heatmap_Relapse) <- annotation_for_heatmap_Relapse$Sample
annotation_for_heatmap_Relapse <- annotation_for_heatmap_Relapse[,-1]
annotation_for_heatmap_Relapse$Response <- ifelse(is.na(annotation_for_heatmap_Relapse$Response), "Relapse_or_No_data", annotation_for_heatmap_Relapse$Response)

png("../Results/RUNX1_CEBPA_activity_Relapse.png", width = 360*(16/9), height = 360)
tf_activities_Relapse %>% as.data.frame() %>% dplyr::select(c("RUNX1", "CEBPA")) %>%
  as.matrix() %>% 
  pheatmap(main="TF activities Relapse",
    show_rownames = FALSE,
    clustering_distance_rows="euclidean",
    scale = "column", 
    annotation_row = annotation_for_heatmap_Relapse,
    annotation_colors = annotation_color_Relapse)
dev.off()
```

```{r}
Sample_no_RUNX1 <- dplyr::filter(Phenotype_Wang_Feng_RNAseq, RUNX1 == "RUNX1_wt") %>% .$Sample

png("../Results/RUNX1_CEBPA_activity_Relapse_no_RUNX1.png", width = 360*(16/9), height = 360)
tf_activities_Relapse %>% as.data.frame() %>% dplyr::select(c("RUNX1", "CEBPA")) %>% .[Sample_no_RUNX1,] %>%
  as.matrix() %>% 
  pheatmap(main="TF activities Relapse",
    show_rownames = FALSE,
    clustering_distance_rows="euclidean",
...
```{r}
png("../Results/TF_activity_Relapse.png", width = 360*(16/9), height = 360)
tf_activities_Relapse %>% as.data.frame() %>% 
  as.matrix() %>% 
  pheatmap(main="TF activities Relapse",
    show_rownames = FALSE,
    clustering_distance_rows="euclidean",
...
```{r}
png("../Results/TF_activity_Relapse_No_RUNX1.png", width = 360*(16/9), height = 360)
tf_activities_Relapse %>% as.data.frame() %>% .[Sample_no_RUNX1,] %>%
  as.matrix() %>% 
  pheatmap(main="TF activities Relapse",
    show_rownames = FALSE,
    clustering_distance_rows="euclidean",
...
```{r}
png("../Results/RUNX1_CEBPA_activity_Relapse_R_No_RUNX1.png", width = 360*(16/9), height = 360)
tf_activities_Relapse %>% as.data.frame() %>% .[unique(c(Sample_relapse_no_RUNX1, Sample_Baseline_R_RUNX1wt)),] %>% dplyr::select(c("RUNX1", "CEBPA")) %>%
  as.matrix() %>% 
  pheatmap(main="TF activities Relapse",
    show_rownames = T,
    clustering_distance_rows="euclidean",
...
```{r}
diff_CEBPA_RUNX1_Relapse <- tf_activities_Relapse %>% as.data.frame() %>% .[c("1825001_BL", "1825001_REL", "2297941_BL", "2297941_REL", "2410163_BL", "2410163_REL"),c("RUNX1", "CEBPA")] 
diff_CEBPA_RUNX1_Relapse$diff <- diff_CEBPA_RUNX1_Relapse$RUNX1 - diff_CEBPA_RUNX1_Relapse$CEBPA

Changement_RUNX1_CEBPA_ratio_at_relapse <- data.frame("Baseline" = c(diff_CEBPA_RUNX1_Relapse[1, 3], diff_CEBPA_RUNX1_Relapse[3, 3], diff_CEBPA_RUNX1_Relapse[5, 3]), "Relapse" = c(diff_CEBPA_RUNX1_Relapse[2, 3], diff_CEBPA_RUNX1_Relapse[4, 3], diff_CEBPA_RUNX1_Relapse[6, 3]))
```

```{r}
plot(c(1,2), Changement_RUNX1_CEBPA_ratio_at_relapse[1,], type = "b", frame = FALSE, pch = 19, 
     col = "red", xlab = "Baseline to Relapse", ylab = "RUNX1/CEBPA activity ratio", ylim = c(-4.5, 2.5), main = "Shift in RUNX1/CEBPA activity ratio after relapse")
lines(c(1,2), Changement_RUNX1_CEBPA_ratio_at_relapse[2,], pch = 18, col = "red", type = "b")
lines(c(1,2), Changement_RUNX1_CEBPA_ratio_at_relapse[3,], pch = 18, col = "blue", type = "b", lty = 2)
```


```{r}
png("../Results/RUNX1_CEBPA_activity_NR_No_RUNX1.png", width = 360*(16/9), height = 360)
tf_activities_Relapse %>% as.data.frame() %>% .[Sample_Baseline_NR,] %>% dplyr::select(c("RUNX1", "CEBPA")) %>%
  as.matrix() %>% 
  pheatmap(main="TF activities NR",
    show_rownames = FALSE,
    clustering_distance_rows="euclidean",
    scale = "column", 
    annotation_row = annotation_for_heatmap_Relapse,
...
```{r}
Phenotype_RNAseq_Responder_Relapse <- ifelse(colnames(RNAseq) %in% Sample_Baseline_R_RUNX1wt, "Responder", ifelse(colnames(RNAseq) %in% Sample_relapse_no_RUNX1, "Relapse", "NA"))

Relapse_Responder_analysis <- Differential_analysis(Phenotype_RNAseq_Responder_Relapse, RNAseq)
```


```{r}
Relapse_Responder_analysis[["Relapse-Responder"]] <- Relapse_Responder_analysis[["Relapse-Responder"]][,c(1,4)]
colnames(Relapse_Responder_analysis[["Relapse-Responder"]]) <- c("logFC_Rel_vs_R", "P.Value_Rel_vs_R")

Genes_values_No_RUNX1m_Responder_Relapse <- merge(Genes_values_No_RUNX1m_Responder_Relapse, Relapse_Responder_analysis[["Relapse-Responder"]], by.x = "Row.names", by.y = 0, all = T)

Genes_values_No_RUNX1m_Responder_Relapse %>% write.csv("../Results/Genes_values_no_RUNX1m_Relapse.csv", row.names = F)
```

```{r}
Sample_no_RUNX1_Baseline <- Sample_no_RUNX1[which(Sample_no_RUNX1 %in% Phenotypes[["Koichi_cohort"]]$Baseline_RNAseq_data)]

tf_activities_no_RUNX1_CEBPa_RUNX1 <- tf_activities_Relapse[Sample_no_RUNX1_Baseline, c("CEBPA", "RUNX1")]

Annotation <- ifelse(rownames(tf_activities_no_RUNX1_CEBPa_RUNX1) %in% Sample_Baseline_NR, "NR", "R")
Annotation <- rep(Annotation, 2)


CEBPA_RUNX1_activity_NR_R_boxplot_df <- data.frame("Activity" = c(tf_activities_no_RUNX1_CEBPa_RUNX1) %>% unlist(),
                                                   "Annotation" = Annotation,
                                                   "TF" = rep(c("CEBPA", "RUNX1"), each = 31))




ggplot(CEBPA_RUNX1_activity_NR_R_boxplot_df, aes(y=Activity, x = Annotation, fill = TF))+
    geom_boxplot(alpha=0.25, outlier.size = -1)+
    geom_jitter(CEBPA_RUNX1_activity_NR_R_boxplot_df, inherit.aes = FALSE, mapping = aes(y = Activity, x = Annotation), width = 0.25, alpha = 0.5, colour = "darkred")+
    ggtitle("Activity of CEBPa and RUNX1 in Responder and Non Responder samples")+
    xlab("")+
    theme(plot.title = element_text(vjust = 2))+
    geom_signif(comparisons = list(c("NR", "R")), map_signif_level=TRUE)+  
    ylab("Relative activity level")+
    scale_shape_manual(values=c(0,1,2,5,6,7))+
    theme(plot.margin=unit(c(0.2,1.5,0.5,0.2),"cm"))
ggsave("../Results/CEBPA_RUNX1_NR_R_activity_boxplot.png")
```

```{r}
Diff_activity_per_sample <- c(tf_activities_no_RUNX1_CEBPa_RUNX1$RUNX1 - tf_activities_no_RUNX1_CEBPa_RUNX1$CEBPA)
names(Diff_activity_per_sample) <- rownames(tf_activities_no_RUNX1_CEBPa_RUNX1)

Annotation_diff <- ifelse(rownames(tf_activities_no_RUNX1_CEBPa_RUNX1) %in% Sample_Baseline_NR, "NR", "R")

CEBPA_RUNX1_diff_activity_NR_R_boxplot_df <- data.frame("Activity" = c(Diff_activity_per_sample),
                                                   "Annotation" = Annotation_diff)


ggplot(CEBPA_RUNX1_diff_activity_NR_R_boxplot_df, aes(y=Activity, x = Annotation, fill = Annotation))+
    geom_boxplot(alpha=0.25, outlier.size = -1)+
    geom_jitter(CEBPA_RUNX1_diff_activity_NR_R_boxplot_df, inherit.aes = FALSE, mapping = aes(y = Activity, x = Annotation), width = 0.25, alpha = 0.5, colour = "darkred")+
    ggtitle("Differencial activity of RUNX1 and CEBPA in Responder and Non Responder samples")+
    xlab("")+
    theme(plot.title = element_text(vjust = 2))+
    geom_signif(comparisons = list(c("NR", "R")), map_signif_level=TRUE)+  
    ylab("RUNX1/CEBPA activity")+
    scale_shape_manual(values=c(0,1,2,5,6,7))+
    theme(plot.margin=unit(c(0.2,1.5,0.5,0.2),"cm"))
ggsave("../Results/Diff_CEBPA_RUNX1_NR_R_activity_boxplot.png")
...
```{r}
Sample_Baseline <- Phenotypes[["Koichi_cohort"]]$Baseline_RNAseq_data %>% na.omit() %>% c()
Sample_RUNX1m_Baseline <- Sample_Baseline[which(Sample_Baseline %in% RUNX1_profile$Sample_name)]
Sample_Baseline_R <- Phenotypes[["Koichi_cohort"]]$Baseline_RNAseq_data[which(Phenotypes[["Koichi_cohort"]]$Baseline_phenotype == "Responder")] %>% na.omit() %>% c()
Sample_Baseline_NR <- Phenotypes[["Koichi_cohort"]]$Baseline_RNAseq_data[which(Phenotypes[["Koichi_cohort"]]$Baseline_phenotype == "Non_Responder")] %>% na.omit() %>% c()


tf_activities_Baseline <- tf_activities_Relapse[Sample_Baseline, c("CEBPA", "RUNX1")]

Annotation_RUNX1_mutation_effect <- ifelse(rownames(tf_activities_Baseline) %in% Sample_RUNX1m_Baseline, "RUNX1m", "RUNX1wt")
Annotation_RUNX1_mutation_effect <- paste(Annotation_RUNX1_mutation_effect, ifelse(rownames(tf_activities_Baseline) %in% Sample_Baseline_R, "R", "NR"), sep = "_")

Annotation_RUNX1_mutation_effect <- rep(Annotation_RUNX1_mutation_effect, 2)



CEBPA_RUNX1_activity_RUNX1m_Response_boxplot_df <- data.frame("Activity" = c(tf_activities_Baseline) %>% unlist(),
                                                   "Annotation" = Annotation_RUNX1_mutation_effect,
                                                   "TF" = rep(c("CEBPA", "RUNX1"), each = 41))




ggplot(CEBPA_RUNX1_activity_RUNX1m_Response_boxplot_df, aes(y=Activity, x = Annotation, fill = TF))+
    geom_boxplot(alpha=0.25, outlier.size = -1)+
    geom_jitter(CEBPA_RUNX1_activity_RUNX1m_Response_boxplot_df, inherit.aes = FALSE, mapping = aes(y = Activity, x = Annotation), width = 0.25, alpha = 0.5, colour = "darkred")+
    ggtitle("Activity of CEBPa and RUNX1 in Responder and Non Responder samples")+
    xlab("")+
    theme(plot.title = element_text(vjust = 2))+
    geom_signif(comparisons = list(c("RUNX1m_R", "RUNX1wt_R"), c("RUNX1m_NR", "RUNX1wt_NR")), map_signif_level=TRUE)+  
    ylab("Relative activity level")+
    scale_shape_manual(values=c(0,1,2,5,6,7))+    
    theme(axis.text.x = element_text(angle = 90)) +
    theme(plot.margin=unit(c(0.2,1.5,0.5,0.2),"cm"))
ggsave("../Results/RUNX1m_on_CE&RUN_activity_boxplot.png")
```

```{r}
tf_activities_Baseline_delta_activity <- tf_activities_Baseline$RUNX1 - tf_activities_Baseline$CEBPA
names(tf_activities_Baseline_delta_activity) <- rownames(tf_activities_Baseline)

Annotation_RUNX1_mutation_effect_delta <- ifelse(names(tf_activities_Baseline_delta_activity) %in% Sample_RUNX1m_Baseline, "RUNX1m", "RUNX1wt")
Annotation_RUNX1_mutation_effect_delta <- paste(Annotation_RUNX1_mutation_effect_delta, ifelse(names(tf_activities_Baseline_delta_activity) %in% Sample_Baseline_R, "R", ifelse(names(tf_activities_Baseline_delta_activity) %in% Sample_Baseline_NR, "NR", "NA")), sep = "_")




delta_activity_RUNX1m_Response_boxplot_df <- data.frame("Activity" = c(tf_activities_Baseline_delta_activity),
                                                   "Annotation" = Annotation_RUNX1_mutation_effect_delta)


ggplot(delta_activity_RUNX1m_Response_boxplot_df, aes(y=Activity, x = Annotation, fill = Annotation))+
    geom_boxplot(alpha=0.25, outlier.size = -1)+
    geom_jitter(delta_activity_RUNX1m_Response_boxplot_df, inherit.aes = FALSE, mapping = aes(y = Activity, x = Annotation), width = 0.25, alpha = 0.5, colour = "darkred")+
    ggtitle("Differencial activity of RUNX1 and CEBPA in RUNX1m and RUNX1wt samples")+
    xlab("")+
    theme(plot.title = element_text(vjust = 2))+
    geom_signif(comparisons = list(c("RUNX1m_R", "RUNX1m_NR"), c("RUNX1wt_NR", "RUNX1wt_R")), map_signif_level=TRUE)+  
    ylab("Relative activity level")+
    scale_shape_manual(values=c(0,1,2,5,6,7))+    
    theme(axis.text.x = element_text(angle = 90)) +
    theme(plot.margin=unit(c(0.2,1.5,0.5,0.2),"cm"))
...
```{r}
RUNX1m_analysis <- ifelse(colnames(RNAseq) %in% Sample_no_RUNX1_Baseline, "RUNX1wt_Baseline", ifelse(colnames(RNAseq) %in% Sample_RUNX1m_Baseline, "RUNX1m_Baseline", "NA"))

RUNX1m_Baseline_analysis <- Differential_analysis(RUNX1m_analysis, RNAseq)

write.csv(RUNX1m_Baseline_analysis[["RUNX1m_Baseline-RUNX1wt_Baseline"]], "../Results/RUNX1m_Baseline_analysis.csv", row.names = T)
```

```{r}
gse_RUNX1m <- Gene_set_analysis(RUNX1m_Baseline_analysis[["RUNX1m_Baseline-RUNX1wt_Baseline"]])
dotplot(gse_RUNX1m, showCategory=10, split=".sign") + facet_grid(.~.sign)
ggsave("../Results/Gene_set_RUNX1mutation_Baseline.png")
```

```{r}
gseaplot(gse_RUNX1m, by = "all", title = "vitamin D binding", geneSetID = 575)
ggsave("../Results/VitaminDbinding_RUNX1m.png")
gseaplot(gse_RUNX1m, by = "all", title = gse_RUNX1m$Description[139], geneSetID = 139)
ggsave("../Results/Vitaminbinding_RUNX1m.png")

```
# Diff TF activities NR vs R

```{r}
A <- rownames(tf_activities) %in% Sample_RUNX1m_Baseline

B <- rownames(tf_activities) %in% Sample_no_RUNX1_Baseline

tf_Ttest_activities_RUNX1m <- tf_activities %>%
  sapply(., function(x){
    test_tmp <- t.test(x[A], x[B])
    c("p.value_activities" = test_tmp$p.value, 
               "Non_reponder" = test_tmp$estimate[1], 
...
write.csv(tf_Ttest_activities_RUNX1m, "../Results/tf_Ttest_activities_RUNX1m.csv")
```


# CEBPA & RUNX1 TF activity Ttest

```{r}
t.test(tf_activities[Sample_Baseline_NR_RUNX1wt, "CEBPA"], tf_activities[Sample_Baseline_R_RUNX1wt, "CEBPA"])
t.test(tf_activities[Sample_Baseline_NR_RUNX1wt, "RUNX1"], tf_activities[Sample_Baseline_R_RUNX1wt, "RUNX1"])
t.test(tf_activities[Sample_RUNX1m_Baseline, "CEBPA"], tf_activities[Sample_no_RUNX1_Baseline, "CEBPA"])
t.test(tf_activities[Sample_RUNX1m_Baseline, "RUNX1"], tf_activities[Sample_no_RUNX1_Baseline, "RUNX1"])

```
# RUNX1/CEBPA activity ratio Ttest

```{r}
t.test(c(tf_activities[Sample_Baseline_NR_RUNX1wt, "RUNX1"] - tf_activities[Sample_Baseline_NR_RUNX1wt, "CEBPA"]) , c(tf_activities[Sample_Baseline_R_RUNX1wt, "RUNX1"] - tf_activities[Sample_Baseline_R_RUNX1wt, "CEBPA"]))

t.test(c(tf_activities[Sample_RUNX1m_Baseline, "RUNX1"] - tf_activities[Sample_RUNX1m_Baseline, "CEBPA"]) , c(tf_activities[Sample_no_RUNX1_Baseline, "RUNX1"] - tf_activities[Sample_no_RUNX1_Baseline, "CEBPA"]))
```

```{r}
Diff_activity_NR_R <- data.frame(Diff_activity = c(c(tf_activities[Sample_Baseline_NR_RUNX1wt, "RUNX1"] - tf_activities[Sample_Baseline_NR_RUNX1wt, "CEBPA"]) , c(tf_activities[Sample_Baseline_R_RUNX1wt, "RUNX1"] - tf_activities[Sample_Baseline_R_RUNX1wt, "CEBPA"])),
                                 Phenotype = c(rep("NR", length(Sample_Baseline_NR_RUNX1wt)), rep("R", length(Sample_Baseline_R_RUNX1wt))))

anno <- t.test(
  c(tf_activities[Sample_Baseline_NR_RUNX1wt, "RUNX1"] - tf_activities[Sample_Baseline_NR_RUNX1wt, "CEBPA"]) , c(tf_activities[Sample_Baseline_R_RUNX1wt, "RUNX1"] - tf_activities[Sample_Baseline_R_RUNX1wt, "CEBPA"])
)$p.value

ggplot(Diff_activity_NR_R, aes(y=Diff_activity, x = Phenotype, fill = Phenotype))+
    geom_boxplot(alpha=0.25, outlier.size = -1)+
    geom_jitter(Diff_activity_NR_R, inherit.aes = FALSE, mapping = aes(y = Diff_activity, x = Phenotype), width = 0.25, alpha = 0.5, colour = "darkred")+
    ggtitle("Differencial activity of RUNX1 and CEBPA in NR and R samples")+
    xlab("")+
    theme(plot.title = element_text(vjust = 2))+
    ...
    theme(axis.text.x = element_text(angle = 90)) +
    theme(plot.margin=unit(c(0.2,1.5,0.5,0.2),"cm"))
ggsave("../Results/RUNX1_CEBPA_diff_NR_vs_R.png")
```
...
ggplot(Diff_activity_NR_R, aes(y=Diff_activity, x = Phenotype, fill = Phenotype))+
    ...
    theme(axis.text.x = element_text(angle = 90)) +
    theme(plot.margin=unit(c(0.2,1.5,0.5,0.2),"cm"))
ggsave("../Results/RUNX1_CEBPA_ratio_NR_vs_R.png")
```