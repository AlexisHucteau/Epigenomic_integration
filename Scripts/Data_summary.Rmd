...
```{r}
DMP_IDHm_IDHwt_TCGA <- BMIQ_TCGA_analysis[["IDHm-WT"]] %>%
  dplyr::filter(., P.Value < 0.01) %>%
  dplyr::filter(., abs(logFC) > 0.3)

Genes_DMP <- Filter_gene_DM_in_promoter(DMP_IDHm_IDHwt_TCGA, match_hit_CpGs_Blueprint_promoter_450)

Genes_DMP_hyper <- Genes_DMP %>%
  dplyr::filter(., logFC > 0)

Genes_DMP_hypo <- Genes_DMP %>%
  dplyr::filter(., logFC < 0)

Look_at_proportions <- merge(number_CpG_per_gene_450, table(Genes_DMP_hyper$Blueprint_gene_names), by.x = "Var1", by.y = "Var1", all.x = TRUE) %>%
  merge(., table(Genes_DMP_hypo$Blueprint_gene_names), by.x = "Var1", by.y = "Var1", all.x = TRUE)


Look_at_proportions$Proportion_hyper <- Look_at_proportions$Freq.y / Look_at_proportions$Freq.x
Look_at_proportions$Proportion_hypo <- Look_at_proportions$Freq / Look_at_proportions$Freq.x
Look_at_proportions$DM <- if(Look_at_proportions$Proportion_hypo < Look_at_proportions$Proportion_hyper){
  "Hyper"
}else if(Look_at_proportions$Proportion_hypo > Look_at_proportions$Proportion_hyper){
  "Hypo"
}else{
  NA
}
