...
```{r}
for (i in gene_list){
  Focus_gene_neighborhood(i, BMIQ_met, matchit_blueprint = match_hit_CpGs_Blueprint, matchit_pchic = matchit_CpGs_Pchic, Phenotype_df = Phenotype_no_CD34, title = "Neighbor_Promoter_methylation_IDHWT_CD34_IDHWT", comparison = c("IDHm","IDHWT"), show_cpgs = TRUE)
}
...
```{r}
for (i in gene_list){
  Focus_gene_neighborhood(i, BMIQ_all, match_hit_CpGs_Blueprint, matchit_CpGs_Pchic, "Promoter_neighborhood_IDHmvsCD34_differential_methylation", Phenotype, filtered_DMP_IDHmvsCD34, diff = TRUE, show_cpgs = TRUE, comparison = c("IDHm","CD34"))
}
for (i in gene_list){
  Focus_gene_neighborhood(i, BMIQ_met, match_hit_CpGs_Blueprint, matchit_CpGs_Pchic, "Promoter_neighborhood_IDHmvsIDHWT_differential_methylation", Phenotype_no_CD34, filtered_DMP_IDHmvsIDHWT, diff = TRUE, show_cpgs = TRUE, comparison = c("IDHm","IDHWT"))
}
for (i in gene_list){
  Focus_gene_neighborhood(i, BMIQ_all, match_hit_CpGs_Blueprint, matchit_CpGs_Pchic, "Promoter_neighborhood_IDHWT_vs_CD34_differential_methylation", Phenotype, filtered_DMP_IDHWT_vs_CD34, diff = TRUE, show_cpgs = TRUE, comparison = c("IDHWT","CD34"))
}
