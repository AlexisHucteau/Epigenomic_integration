...
```{r}
Analysis_DGEs_cell_line <- DGEs(Transcriptomes$Transcriptomes, Transcriptomes$Phenotype$Characteristics.cell.line., Transcriptomes$Annot)
...
```{r}
volcanoplot_gene_expression(Analysis_DGEs_cell_line$`HL60-MOLM14`, "HL60_vs_MOLM14", ylim = 30)
```
...
TF_activities[["HL60_Tf_Activities"]] <- tf_activities %>%
  .[Transcriptomes$Phenotype$Name[which(Transcriptomes$Phenotype$Characteristics.cell.line. == "HL60")],] %>%
  as.matrix(.) %>%
  colMeans2(.) %>%
  as.data.frame(.)
